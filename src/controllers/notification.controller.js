...
exports.sendEmail = async (req, res) => {
  try {
    const { emails, emailType, reason, email, password, htmlContent } =
      req.body;
      ...
      case "funded_login":
        if (!email || !password) {
          return res.status(400).json({
            message: "Email and password are required for this email type",
          });
        }
        break;
    ...
    sendSmtpEmail.subject = "Notification from Globby Funded Traders"; // subject can also come from frontend if needed
